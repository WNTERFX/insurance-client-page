  ...
  try {
    const { data, error } = await db
      ...
      .select(`
        ...
        paymongo_transactions (
         ...
         paymongo_transactions:paymongo_transactions!paymongo_transactions_payment_id_fkey (
          reference_number
        )
      `)
      .eq("is_paid", true)
    ...
    const formattedData = (data || []).map((payment) => {
      const penaltiesTotal =
        (payment.payment_due_penalties || []).reduce(
          (sum, p) => sum + (p.is_paid ? Number(p.penalty_amount) || 0 : 0),
          0
        ) || 0;

      const refNo =
        (payment.paymongo_transactions?.[0]?.reference_number) ||
      ...
      return {
        ...
        // keep number; format in the component to avoid "Php NaN"
        amount: Number(payment.paid_amount ?? payment.amount_to_be_paid) || 0,
        company:
          payment.policy_Table?.insurance_Partners?.insurance_Name || "N/A",
        ...
        clientRegistered: payment.policy_Table?.clients_Table?.client_Registered
          ? new Date(
            payment.policy_Table.clients_Table.client_Registered
